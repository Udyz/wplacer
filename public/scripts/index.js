...
const changeTab = (el) => {
    ...
    if (currentTab === griefers) {
        // Load griefers when switching to griefers tab
        loadGriefers();
    }
...
const MODE_PREVIEW = (() => {
    ...
    function baseOrderForMode(mode, target, scene) {
        switch (mode) {
            ...
            case 'linear-skip1': {
                const even = target.filter(p => ((p.x + p.y) % 2) === 0);
                return orderByLinear(even, 'y', false);
            }
            case 'linear-skip1-fill': {
                const even = [], odd = [];
                for (const p of target) (((p.x + p.y) % 2) === 0 ? even : odd).push(p);
                return [...orderByLinear(even, 'y', false), ...orderByLinear(odd, 'y', false)];
            }
            case 'linear-ltr': return orderByLinear(target, 'x', false);
            case 'linear-ltr-skip1': {
                const even = target.filter(p => ((p.x + p.y) % 2) === 0);
                return orderByLinear(even, 'x', false);
            }
...
// --- Grief Monitoring Functions ---
let currentGriefersData = null;
let griefMonitoringEnabled = false;

async function loadGriefers() {
    try {
        const response = await axios.get('/griefers');
        const data = response.data;

        if (data.success) {
            currentGriefersData = data.data;
            griefMonitoringEnabled = data.monitoringEnabled || false;
            updateGriefersTable(data.data);
            updateGriefersSummary(data.data);
            updateGriefersMonitoringStatus(data.monitoringEnabled, data.monitoringActive);
            
            // Log when griefers are loaded from file
            if (data.data && data.data.length > 0) {
                console.log(`Loaded ${data.data.length} griefers from server`);
            }
        } else {
            console.error('Failed to load griefers:', data.error);
            showGriefersError('Failed to load griefers data');
        }
    } catch (error) {
        console.error('Error loading griefers:', error);
        showGriefersError('Error loading griefers data');
    }
}

function updateGriefersSummary(griefers) {
    const griefersTotalCount = document.getElementById('griefersTotalCount');
    const griefersLastDetection = document.getElementById('griefersLastDetection');
    
    if (griefersTotalCount) {
        griefersTotalCount.textContent = griefers.length;
    }
    
    if (griefersLastDetection) {
        if (griefers.length > 0) {
            const lastGriefer = griefers.reduce((latest, current) => {
                return new Date(current.detectedAt) > new Date(latest.detectedAt) ? current : latest;
            });
            griefersLastDetection.textContent = new Date(lastGriefer.detectedAt).toLocaleString();
        } else {
            griefersLastDetection.textContent = '–';
        }
    }
}

function updateGriefersMonitoringStatus(enabled, active) {
    const griefersMonitoringStatus = document.getElementById('griefersMonitoringStatus');
    const griefMonitoringToggle = document.getElementById('griefMonitoringToggle');
    
    if (griefersMonitoringStatus) {
        if (enabled && active) {
            griefersMonitoringStatus.textContent = 'Active';
            griefersMonitoringStatus.style.color = 'var(--success)';
        } else if (enabled && !active) {
            griefersMonitoringStatus.textContent = 'Starting...';
            griefersMonitoringStatus.style.color = 'var(--warning)';
        } else {
            griefersMonitoringStatus.textContent = 'Disabled';
            griefersMonitoringStatus.style.color = 'var(--muted)';
        }
    }
    
    if (griefMonitoringToggle) {
        griefMonitoringToggle.checked = enabled;
    }
}

function updateGriefersTable(griefers) {
    const griefersTable = document.getElementById('griefersTable');
    
    if (!griefersTable) return;
    
    if (griefers.length === 0) {
        griefersTable.innerHTML = `
            <div class="griefers-loading">
                <img src="icons/eye.svg" alt="" style="width: 32px; height: 32px; opacity: 0.5; margin-bottom: 8px;" />
                <p>No griefers detected</p>
            </div>
        `;
        return;
    }
    ...
    const tableHTML = `
        <table>
            ...
            <tbody>
                ${griefers.map(griefer => `
                    <tr>
                        <td class="griefer-name">${griefer.name}</td>
                        <td class="griefer-id">#${griefer.id}</td>
                        <td class="griefer-alliance">${griefer.alliance || '–'}</td>
                        <td>
                            <span class="griefer-type ${griefer.griefType}">${griefer.griefType}</span>
                        </td>
                        <td>
                            ${griefer.url ? `<a href="${griefer.url}" target="_blank" class="griefer-url">🗺️</a>` : '–'}
                        </td>
                        <td class="griefer-template">${griefer.templateName || 'Unknown'}</td>
                        <td class="griefer-time">${new Date(griefer.detectedAt).toLocaleString()}</td>
                        <td class="griefer-actions">
                            <button onclick="sendReport('${griefer.id}')" title="Send Report" class="report-btn"><img src="icons/email.svg" alt="" /></button>
                            <button onclick="removeGriefer('${griefer.id}')" title="Remove">×</button>
                        </td>
                    </tr>
                `).join('')}
            </tbody>
    ...
    griefersTable.innerHTML = tableHTML;
}

function showGriefersError(message) {
    const griefersTable = document.getElementById('griefersTable');
    if (griefersTable) {
        griefersTable.innerHTML = `
            <div class="griefers-loading" style="color: var(--error);">
                <p>${message}</p>
            </div>
        `;
    }
...
async function clearAllGriefers() {
    try {
        const response = await axios.delete('/griefers');
        if (response.data.success) {
            console.log('Successfully cleared all griefers');
            await loadGriefers();
        } else {
            console.error('Failed to clear griefers:', response.data.error);
            alert('Failed to clear griefers: ' + (response.data.error || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error clearing griefers:', error);
        alert('Failed to clear griefers: ' + error.message);
    }
}

async function removeGriefer(grieferId) {
    try {
        const response = await axios.delete(`/griefers/${grieferId}`);
        if (response.data.success) {
            console.log(`Successfully removed griefer ${grieferId}`);
            await loadGriefers();
        } else {
            console.error('Failed to remove griefer:', response.data.error);
            alert('Failed to remove griefer: ' + (response.data.error || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error removing griefer:', error);
        alert('Failed to remove griefer: ' + error.message);
    }
}

function sendReport(grieferId) {
    // Find the griefer data - convert grieferId to number for comparison
    const griefer = currentGriefersData?.find(g => g.id === parseInt(grieferId) || g.id === grieferId);
    if (!griefer) {
        showConfirmation('Griefer not found');
        return;
    }
    
    // Fake report - just show alert
    showMessageBig('Griefer report', `📧 Report sent for griefer: ${griefer.name}#${griefer.id}\n\nLocation: ${griefer.lat}, ${griefer.lng}\nAlliance: ${griefer.alliance || 'None'}\nGrief Type: ${griefer.griefType}\nDetected: ${new Date(griefer.detectedAt).toLocaleString()}\n\n(This is a fake report - no actual action taken)`);
    
    console.log(`📧 Fake report sent for griefer:`, griefer);
}

async function exportGriefersCSV() {
    try {
        const response = await axios.get('/griefers/export/csv', {
            responseType: 'blob'
        });
        ...
        link.href = url;
        link.setAttribute('download', 'griefers_export.csv');
...
async function toggleGriefMonitoring(enabled) {
    try {
        console.log(`🔄 Toggling grief monitoring: ${enabled ? 'ON' : 'OFF'}`);
        const response = await axios.post('/griefers/monitoring', { enabled });
        console.log('Response:', response.data);
        
        if (response.data.success) {
            griefMonitoringEnabled = enabled;
            updateGriefersMonitoringStatus(response.data.monitoringEnabled, response.data.monitoringActive);
            console.log(`✅ Grief monitoring ${enabled ? 'enabled' : 'disabled'}`);
        } else {
            console.error('❌ Failed to toggle grief monitoring:', response.data);
        }
    } catch (error) {
        console.error('❌ Error toggling grief monitoring:', error);
        alert('Failed to toggle grief monitoring: ' + error.message);
    }
}

// Event listeners for grief monitoring controls
document.addEventListener('DOMContentLoaded', () => {
    const clearGriefersBtn = document.getElementById('clearGriefersBtn');
    const refreshGriefersBtn = document.getElementById('refreshGriefersBtn');
    const griefMonitoringToggle = document.getElementById('griefMonitoringToggle');
    
    if (clearGriefersBtn) {
        clearGriefersBtn.addEventListener('click', clearAllGriefers);
    }
    
    if (refreshGriefersBtn) {
        refreshGriefersBtn.addEventListener('click', loadGriefers);
    }
    
    const exportGriefersBtn = document.getElementById('exportGriefersBtn');
    if (exportGriefersBtn) {
        exportGriefersBtn.addEventListener('click', exportGriefersCSV);
    }
    
    if (griefMonitoringToggle) {
        griefMonitoringToggle.addEventListener('change', (e) => {
            toggleGriefMonitoring(e.target.checked);
        });
    }
    
    // Load griefers on page load
    loadGriefers();
    
    // Auto-refresh griefers data every 30 seconds
    setInterval(() => {
        if (currentTab === griefers) {
            loadGriefers();
        }
    ...
    // Check server health and reload griefers if needed
    let lastServerCheck = Date.now();
    setInterval(async () => {
        try {
            const response = await axios.get('/griefers');
            if (response.data.success) {
                // If we have griefers data but currentGriefersData is empty, reload
                if (response.data.data && response.data.data.length > 0 && 
                    (!currentGriefersData || currentGriefersData.length === 0)) {
                    console.log('Server restarted - reloading griefers data');
                    loadGriefers();
                }
            }
        } catch (error) {
            // Server might be restarting, will retry next time
        }
